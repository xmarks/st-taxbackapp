<div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats['receipt_count'] }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Total VAT Amount</p>
                                <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ number_format($stats['total_vat_amount'], 2) }} ALL</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Recent Receipts -->
            @if($stats['recent_receipts']->isNotEmpty())
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($stats['recent_receipts']->take(5) as $receipt)
                                <tr wire:key="dashboard-receipt-{{ $receipt->id }}">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                        <div class="font-medium">{{ $receipt->seller->name }}</div>
                                        <div class="text-gray-500 dark:text-gray-400">{{ $receipt->seller->town }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                        {{ number_format($receipt->total_price, 2) }} ALL
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        {{ $receipt->scanned_at->format('M j, Y') }}
                                    </td>
                    ...
                    @if($stats['recent_receipts']->count() > 5)
                    <div class="mt-4">
                        <a href="{{ route('receipts.index') }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">View all {{ $stats['receipt_count'] }} receipts →</a>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-center">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No receipts</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by scanning your first receipt.</p>
                    <div class="mt-6">
                        <a href="{{ route('receipts.scan') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            Scan Receipt
                        </a>
